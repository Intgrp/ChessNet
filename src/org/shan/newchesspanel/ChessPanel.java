...
public class ChessPanel extends Panel implements MouseListener {
	...
	public JTextField statusText = new JTextField("请先连接服务器");//显示客户端状态的文本框
	...
	public List<String> subcribe = new ArrayList<String>();
	
	/**
	 * 增加的函数，用于广播给所有订阅方，发布方的棋盘信息
	 * 消息格式，/publish 来源 订阅方 黑棋坐标 白棋坐标（用逗号分隔各个坐标）
	 * 形如  黑棋x1,黑棋y1,黑棋x2,黑棋y2;白棋x1,白棋y1,白棋x2,白棋x2
	 */
	public String chessToString() {
		String sndMessage="";
		for (int i=0; i<chessBlack_x.length-1; i++) {
			sndMessage+=String.valueOf(chessBlack_x[i])+","+String.valueOf(chessBlack_y[i])+",";
		}
		sndMessage+=String.valueOf(chessBlack_x[chessBlack_x.length-1])+","+String.valueOf(chessBlack_y[chessBlack_x.length-1])+";";
		
		for (int i=0; i<chessWhite_x.length-1; i++) {
			sndMessage+=String.valueOf(chessWhite_x[i])+","+String.valueOf(chessWhite_y[i])+",";
		}
		sndMessage+=String.valueOf(chessWhite_x[chessWhite_x.length-1])+","+String.valueOf(chessWhite_y[chessWhite_x.length-1]);
		return sndMessage;
//		chessthread.sendMessage(sndMessage);
	}
	/**
	 * 观战时用于获得他人的棋盘信息，更新状态
	 * @param chessBlack_x
	 ...
	 * @param chessWhite_y
	 */
	public void subcribeChess(int chessBlack_x[], int chessBlack_y[], int chessWhite_x[], int chessWhite_y[] ) {
		for (int i=0; i<chessBlack_x.length; i++) {
			netChessPaint(chessBlack_x[i], chessBlack_y[i], 1);
			netChessPaint(chessWhite_x[i], chessWhite_y[i], -1);
		}
	}
	...
	public ChessPanel() {
		setSize(440, 440);
		setLayout(null);
		setBackground(new Color(211, 168, 76));
