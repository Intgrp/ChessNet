 ...
 * 显示服务器及用户信息的Panel类
 */
class MessageMultyServerPanel extends Panel {
 ...
 */
class ServerMultyThread extends Thread {
	Socket clientSocket;
	...
	MessageMultyServerPanel server;
	 ...
	 */
	ServerMultyThread(Socket clientSocket, Hashtable clientDataHash, Hashtable clientNameHash, Hashtable chessPeerHash,
			MessageMultyServerPanel server) {
		...
		if (message.startsWith("/")) {
			...
			if (message.startsWith("/r")) {
				...
				if (clientName.length() <= 0 || clientName.length() > 20 || clientName.startsWith("/")
						...
						//增加的属性
						|| clientName.startsWith("attachsub") || clientName.startsWith("connattach")
						|| clientName.startsWith("chessall")) {
					...
					// 如果用户在对弈双方的服务端，做如下处理。
						...
						synchronized (chessPeerHash) {
							// chessPeerHash重新映射
							chessPeerHash.put((String) getHashKey(chessPeerHash, clientNameHash.get(clientSocket)),
									clientName);
							// 向游戏客户端发送新名字
							chessPeerTalk(("[inchess]" + clientName), ("/yourname " + ("[inchess]" + clientName)));
						}
			...
			// 然后修改clientNameHash表和chessPeerHash表。
				...
				if (chessPeerHash.containsKey(serverName) && chessPeerHash.get(serverName).equals("wait")) {
					...
					chessPeerTalk(serverName, "/connattach "+selfName);
					try {
						clientClose();
					} catch (Exception ez) {
					}
			...
			// 增加的订阅功能，客户端发送订阅某用户下棋情况，消息为 /attachsub 对方 订阅方
			else if (message.startsWith("/attachsub ")) {
				StringTokenizer userToken = new StringTokenizer(message, " ");
				String getUserToken, serverName, selfName;
				String[] chessNameOpt = { "0", "0" };
				int getOptNum = 0;
			...
			//如果以/chessall开头，则向消息里面对应的用户发送消息来源处整个其他信息
			else if (message.startsWith("/chessall")) {//未完
				StringTokenizer userToken = new StringTokenizer(message, " ");
				String getUserToken, serverName, selfName;
				String[] chessNameOpt = { "0", "0" };
				int getOptNum = 0;
	...
	/**
	 * 发送棋盘更新消息的函数，将消息向指定列表的客户端都发送一份
	 */
	public void publicSubscribeChess(String publicTalkMessage) {
 ...
 */
public class ChessMultyServer extends Frame implements ActionListener {
	...
	MessageMultyServerPanel server = new MessageMultyServerPanel();
	 ...
	 * 初始化消息服务器的类
	 */
	public void makeMessageServer(int port, MessageMultyServerPanel server) throws IOException {
		Socket clientSocket;
		...
		try {
			...
			while (true) {
				...
				clientNameHash.put(clientSocket, ("新游客" + clientAccessNumber++));
				ServerMultyThread thread = new ServerMultyThread(clientSocket, clientDataHash, clientNameHash,
						chessPeerHash, server);
				thread.start();
			}
	...
	public static void main(String args[]) {
		...
		ChessMultyServer ChessServer = new ChessMultyServer();
	}
}